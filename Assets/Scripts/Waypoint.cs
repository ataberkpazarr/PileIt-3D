...
public class Waypoint : MonoBehaviour //Vector 3 positions in space
{
    ...
    public Vector3 GetWaypointPosition(int current,int target) // returning the index certain waypoint which corresponds to next position in enemy's movement
    {

        return Points[current] + Points[target];
